...
//#endif
        ...
        /// <summary>
        /// 读取 CSV：首行为表头（跳过）。每行第 1 列为 string key，第 2 列起每个单元格解析为 T，按列顺序放入 List&lt;T&gt;。
        /// 若同一 key 出现多行，后行会覆盖前行并打 Warning。
        /// </summary>
        public static Dictionary<string, List<T>> LoadListDictionary<T>(string filename)
        {
            var ret = new Dictionary<string, List<T>>();
            var ta = Resources.Load<TextAsset>("csv/" + filename);
            if (ta == null || string.IsNullOrEmpty(ta.text))
            {
                Debug.LogError("CsvUtil.LoadListDictionary: cannot load csv/" + filename);
                return ret;
            }
            ...
            while (lineId < lines.Length - 1)
            {
                ...
                if (ret.ContainsKey(key))
                    Debug.LogWarning("CsvUtil.LoadListDictionary: duplicate key '" + key + "' in " + filename + ", overwritten.");
                ret[key] = list;
            }
